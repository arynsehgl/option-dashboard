...
export async function fetchOptionChainData(symbol = 'NIFTY') {
  ...
  throw new Error('No API backend configured. Please enable Netlify Functions or Express Server.');
}
...
async function fetchViaNetlifyFunction(symbol) {
  ...
  try {
    ...
    if (response.status === 404) {
      if (isLocalDev) {
        throw new Error('Netlify Function not found. To test functions locally, run "npm run dev:netlify" instead of "npm run dev".');
      }
      throw new Error('Netlify Function not found (404). Make sure the function is deployed on Netlify.');
    }
    ...
    if (!contentType || !contentType.includes('application/json')) {
      ...
      if (isLocalDev && text.includes('<!doctype') || text.includes('<!DOCTYPE')) {
        throw new Error('Netlify Functions are not available. Run "npm run dev:netlify" to test functions locally, or deploy to Netlify for production.');
      }
      
      throw new Error(`Unexpected response type: ${contentType || 'unknown'}. The function may not be working correctly.`);
    }
    ...
    if (!result.data || !result.data.records || !result.data.records.data || result.data.records.data.length === 0) {
      throw new Error('Netlify function returned empty data. The NSE API may not be responding or the data structure changed.');
    }
